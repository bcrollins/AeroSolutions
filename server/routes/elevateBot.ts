/**
 * ROLLINSXBot AI Assistant - Specialized business-focused chatbot endpoints
...
/**
 * Handles business queries from the ROLLINSXBot chat interface
 */
export async function handleROLLINSXBotQuery(req: Request, res: Response) {
  try {
    ...
    if (cachedResponse) {
      console.log('ROLLINSXBot cache hit');
    ...
    console.log('ROLLINSXBot cache miss, generating response with OpenAI');
    
    // Create the system prompt for business-focused responses
    const businessSystemPrompt = `You are ROLLINSXBot, a tech assistant from ROLLINSX, a premier web development company for small businesses. 
...
3. Highlight ROLLINSX's focus on affordable, high-quality web solutions specifically tailored for small businesses.
4. Mention ROLLINSX's competitive pricing when relevant (approximately 60% below market rates).
...
8. Emphasize ROLLINSX's AI-powered approach that makes professional web development accessible to small businesses.

Remember that ROLLINSX specializes in web development with these core services:
...
- AI-driven business insights`;
    ...
    console.error("ROLLINSXBot error:", error);
    const errorMessage = error instanceof Error ? error.message : "Unknown error occurred";
    
    res.status(500).json({
      success: false,
      message: "Failed to process ROLLINSXBot request",
      error: errorMessage
    });
  }
}
 ...
 */
export async function handleROLLINSXBotQuerySimple(req: Request, res: Response) {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    const businessInstructions = 
      "You are ROLLINSXBot, a tech assistant from ROLLINSX, a web development company for small businesses. " +
      "Provide expert but accessible advice about web development, site design, and digital presence. " +
      "Highlight ROLLINSX's focus on affordable web solutions (60% below market rates). " +
    ...
    try {
      ...
      console.error("ROLLINSXBot AI generation error:", error);
